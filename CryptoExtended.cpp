...
namespace hexwallet
{
    namespace
    {
        ...
        void sha512_compress(uint64_t state[8], const uint8_t block[128])
        {
            ...
            {
                const uint64_t s0 = rotate_right64(w[word - 15], 1) ^
                                    rotate_right64(w[word - 15], 8) ^ (w[word - 15] >> 7);
                const uint64_t s1 = rotate_right64(w[word - 2], 19) ^
                                    rotate_right64(w[word - 2], 61) ^ (w[word - 2] >> 6);
    ...
    bool crypto_blake2b(const uint8_t *data, size_t size, uint8_t *out,
                        size_t out_size)
    {
        if (out == nullptr || out_size == 0 || out_size > 64 ||
            (data == nullptr && size != 0))
        {
            return false;
        }
...
#undef BLAKE2B_G2
                       ...
                       char *out, size_t out_size, bool bech32m)
    {
        ...
        if (hrp_size == 0 || hrp_size > 83 || data_size > 200 ||
            out_size < hrp_size + 1 + data_size + 6 + 1)
        {
            return false;
        }
        ...
        {
            out[written++] =
                kBase32Charset[static_cast<uint8_t>((checksum >> (5 * (5 - index))) &
                                                    0x1f)];
        }
        out[written] = '\0';
        return true;
    ...
    bool run_crypto_extended_self_tests()
    {
        ...
        bool passed = selftest_report("sha3 empty", crypto_sha3_256(nullptr, 0, digest) &&
                      crypto_constant_time_equal(digest, kSha3EmptyExpected, 32));
        passed = selftest_report("sha3 abc", crypto_sha3_256(kAbc, sizeof(kAbc), digest) &&
                 crypto_constant_time_equal(digest, kSha3AbcExpected, 32)) && passed;
        ...
        passed = selftest_report("sha512/256 empty", crypto_sha512_256(nullptr, 0, digest) &&
                 crypto_constant_time_equal(digest, kSha512256Empty, 32)) && passed;
        passed = selftest_report("sha512/256 abc", crypto_sha512_256(kAbc, sizeof(kAbc), digest) &&
                 crypto_constant_time_equal(digest, kSha512256Abc, 32)) && passed;
        ...
        uint8_t blake64[64];
        passed = selftest_report("blake2b-512 abc", crypto_blake2b(kAbc, sizeof(kAbc), blake64, sizeof(blake64)) &&
                 crypto_constant_time_equal(blake64, kBlake2b512Abc, sizeof(blake64))) && passed;
        ...
        passed = selftest_report("blake2b-256 abc", crypto_blake2b(kAbc, sizeof(kAbc), digest, 32) &&
                 crypto_constant_time_equal(digest, kBlake2b256Abc, 32)) && passed;

        // RFC 4648 base32.
        char encoded[128];
        static const uint8_t kBase32Input[] = {'f', 'o', 'o'};
        passed = selftest_report("base32 foo", base32_encode(kBase32Input, sizeof(kBase32Input), encoded, sizeof(encoded), false, true) &&
                 strcmp(encoded, "MZXW6===") == 0) && passed;
        ...
        char cashaddr[128];
        passed = selftest_report("cashaddr", cashaddr_encode("bitcoincash", 0, kCashPayload, sizeof(kCashPayload), cashaddr, sizeof(cashaddr)) &&
                 strcmp(cashaddr,
                        "bitcoincash:qw6syq5aa5z5ghkj3w7ux59wrk204txrnxjeq7f94") == 0) && passed;
        ...
        passed = selftest_report("bech32", bech32_encode("bc", kBech32Versioned, sizeof(kBech32Versioned), bech32, sizeof(bech32), false) &&
                 strcmp(bech32, "bc1qp63uahgrxged4z5jswyt5dn5v3lzsem6c0qqhg8") == 0) && passed;
        ...
        passed = selftest_report("bech32m", bech32_encode("bc", kBech32mVersioned, sizeof(kBech32mVersioned), bech32m, sizeof(bech32m), true) &&
                 strcmp(bech32m,
                        "bc1q9umuen7l8wthtz45p3ftn58pvrs9xlumvkuu2xet8egzkcklqtesq0reqa") ==
                     0) && passed;

        // Kaspa bech32 (kaspa-addresses test vectors).
        uint8_t zeros[32] = {0};
        char kaspa[128];
        passed = selftest_report("kaspa zero", kaspa_bech32_encode("kaspa", 0, zeros, sizeof(zeros), kaspa, sizeof(kaspa)) &&
                 strcmp(kaspa,
        ...
        passed = selftest_report("kaspa payload", kaspa_bech32_encode("kaspa", 0, kKaspaPayload, sizeof(kKaspaPayload), kaspa, sizeof(kaspa)) &&
                 strcmp(kaspa,
        ...
        char qubic[64];
        passed = selftest_report("qubic zero-hash", qubic_base26_encode(zero_hash, sizeof(zero_hash), qubic, sizeof(qubic)) &&
                 strcmp(qubic,
                        "3mj9k21j99ekmh8bg8k96drkf6m7g84f1j6h833ok47ph13kefajkdb") == 0) && passed;
        ...
        uint8_t ed_pk[32];
        uint8_t ed_sig[64];
        passed = selftest_report("ed25519 TC1 pk", ed25519_public_key(kEdSeed1, ed_pk) &&
                 crypto_constant_time_equal(ed_pk, kEdPk1, 32)) && passed;
        passed = selftest_report("ed25519 TC1 sig", ed25519_sign(kEdSeed1, nullptr, 0, ed_sig) &&
                 crypto_constant_time_equal(ed_sig, kEdSig1, 64)) && passed;
        ...
        passed = selftest_report("ed25519 TC2 pk", ed25519_public_key(kEdSeed2, ed_pk) &&
                 crypto_constant_time_equal(ed_pk, kEdPk2, 32)) && passed;
        passed = selftest_report("ed25519 TC2 sig", ed25519_sign(kEdSeed2, kEdMsg2, sizeof(kEdMsg2), ed_sig) &&
                 crypto_constant_time_equal(ed_sig, kEdSig2, 64)) && passed;
        ...
        passed = selftest_report("slip10 child 0'", slip10_ed25519_derive(kSlipSeed, sizeof(kSlipSeed), kSlipPath, 1, slip_child) &&
                 crypto_constant_time_equal(slip_child, kSlipChild, 32)) && passed;
        ...
        uint8_t bls_pk[48];
        passed = selftest_report("bls pk(sk=1)", bls12_381_g1_public_key(kBlsOne, bls_pk) &&
                 crypto_constant_time_equal(bls_pk, kBlsGenerator, 48)) && passed;
        ...
        passed = selftest_report("eip2333 master", eip2333_master_key(kEipSeed, sizeof(kEipSeed), eip_master) &&
                 crypto_constant_time_equal(eip_master, kEipMaster, 32)) && passed;
        passed = selftest_report("eip2333 child0", eip2333_derive_child(eip_master, 0, eip_child) &&
                 crypto_constant_time_equal(eip_child, kEipChild, 32)) && passed;
        ...
        char chia[128];
        passed = selftest_report("chia addr v0", chia_standard_address(kEipSeed, sizeof(kEipSeed), 0, chia, sizeof(chia)) &&
                 strcmp(chia,
                        "xch1sqh8a9jwxfvh0rvzza9sslzjgvhjfqcrq47kwf3hq7sx00pfnw2q6pe2gr") ==
                     0) && passed;
        // Chia with a 64-byte 0x01 seed.
        uint8_t ones_seed[64];
        memset(ones_seed, 0x01, sizeof(ones_seed));
        passed = selftest_report("chia addr ones", chia_standard_address(ones_seed, sizeof(ones_seed), 0, chia, sizeof(chia)) &&
                 strcmp(chia,
                        "xch1wv0k6nwah2vx6rgplqcey8yhcv3vd2sadwjx6gpa9n9wkcx4p48q6v7n2d") ==
                     0) && passed;
