...
void compress(uint32_t state[kDigestWords], const uint8_t block[kBlockSize]) {
  ...
  for (uint8_t round = 0; round < kRounds; ++round) {
    ...
    uint32_t next = rotate_left(al + round_function(group, bl, cl, dl) + words[kLeftIndex[round]] + kLeftConstants[group],
                                kLeftShift[round])
                    + el;
    al = el;
    el = dl;
    ...
    cl = bl;
    bl = next;
    next = rotate_left(ar + round_function(4 - group, br, cr, dr) + words[kRightIndex[round]] + kRightConstants[group],
                       kRightShift[round])
           + er;
    ar = er;
    er = dr;
